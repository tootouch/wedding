...
## 축하 메시지 Google Sheets 연결

축하 메시지는 `script.js`의 `guestbookEndpoint` 값이 설정되면 Google Sheets로 전송됩니다.

1. Google Sheets에서 새 스프레드시트를 만듭니다.
2. 메뉴에서 `확장 프로그램 > Apps Script`를 엽니다.
3. 이 저장소의 `google-apps-script-guestbook.gs` 내용을 Apps Script 편집기에 붙여넣습니다.
...
URL을 넣은 뒤 다시 commit/push하면 GitHub Pages의 축하 메시지가 `GUESTBOOK` 시트에 기록됩니다.
